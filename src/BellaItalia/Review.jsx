...
const Review = () => {
 ...
 <div className='flex flex-col gap-4'>
    {visiblecards.map((item,index)=>(
        <div key={index}  className='flex flex-col gap-4 bg-[#F8F8F8] p-4 lg:p-5 rounded-md'>
        <div className='flex  gap-2.5'>
         <div className=''>
             <img className=' py-3 px-3 bg-white rounded-full w-fit ' src={item.Logo}/>
         </div>
            <div>
            <h2 className=' lg:text-[21px] md:text-lg text-base text-[#232323] font-semibold'>{item.title}</h2>
            <p className='lg:text-base md:text-sm text-[12px] text-[#8A8A8A] '>{item.location}</p>
            ...
            <p className='text-[#343434] text-sm'>{item.date}</p>
             ...
             <div>
                <p className='lg:text-base md:text-sm text-[12px] text-[#5E5E5E] max-w-[511px] '>{item.des}</p>
             </div>
  ...
  {hasmore &&(
     ...
     <button onClick={handleSeeMoro} className='bg-[#1677BD] py-2 px-4 rounded-full text-white'>
        Show more reviews
    </button>
   </div>
  )}
